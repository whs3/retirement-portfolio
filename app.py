...
# ── API: Market Insights + Analyst Recommendations ────────────────────────────
...
def get_insights():
    ...
    # Fetch market snapshot once (cheap indices)
    ...
    # Fetch analyst data in parallel (skip pure cash/no-ticker entries for API cost)
    analyst_results: dict[str, dict] = {}
    fetch_tickers = [k for k, v in ticker_value.items() if v["ticker"] and v["ticker"] != "$$CASH"]

    if fetch_tickers:
        with ThreadPoolExecutor(max_workers=8) as pool:
            futures = {pool.submit(_get_analyst_snapshot, tk): tk for tk in fetch_tickers}
    ...
    # Build per-ticker rows for the UI
    holdings_out = []
    covered_value = 0.0
    upside_weighted_sum = 0.0
    upside_weight_sum = 0.0
    sentiment: dict[str, float] = {}   # rating -> total value
    ...
    # Remove holdings with zero or negative value (e.g. closed positions or net sells)
    holdings_out = [h for h in holdings_out if h.get("current_value", 0) > 0]

    # Sort table by value desc by default
    ...
    # Sentiment breakdown (nice order)
    ...
    # --- Fund / ETF characteristics aggregates (very useful when most holdings are funds) ---
    ...
    fund_stats = {}
    if expense_weight > 0:
        fund_stats["weighted_avg_expense"] = round(expense_weighted / expense_weight, 3)  # raw number e.g. 0.094
    if pe_weight > 0:
    ...
    # Generate high-level recommendations (server side for consistency)
    recommendations = []

    if avg_upside is not None:
        sign = "positive" if avg_upside >= 0 else "negative"
        recommendations.append(
            f"Analyst consensus implies an average {sign} upside of {abs(avg_upside):.1f}% across analyst-covered holdings."
        )
    ...
    if buy_value > total_value * 0.15:
        recommendations.append(
            f"{len([h for h in holdings_out if h.get('action_code') == 'add'])} holdings currently show strong analyst upside (>10% and Buy/Strong Buy)."
        )
    if sell_value > 0:
        recommendations.append(
            f"You have ${sell_value:,.0f} in holdings with Sell or Strong Sell consensus — review these positions."
        )

    # Fund/ETF specific signals (often the only signals available)
    ...
    if fund_stats.get("weighted_avg_pe"):
        ...
        if w_pe > 26:
            recommendations.append(
                ...
                "This may favor tilting toward lower-valuation areas (value, international, or broad market)."
            )
        elif w_pe < 18:
            recommendations.append(
                f"Equity holdings appear relatively inexpensive on valuation (weighted avg P/E ≈ {w_pe})."
            )
    ...
    if total_value > 0 and covered_value / total_value < 0.4:
        recommendations.append(
            "A large portion of your portfolio has limited or no analyst coverage (ETFs, funds, bonds, or cash). Analyst signals apply only to the covered slice."
        )
    ...
    if vix and vix.get("price"):
        ...
        recommendations.append(f"Current VIX level ({vix['price']:.1f}) indicates {vol_note} market volatility.")
...
# ── Analyst + Market helpers for Insights page ────────────────────────────────
...
def _get_analyst_snapshot(ticker: str) -> dict:
    ...
    try:
        ...
        # Fund / ETF characteristics (useful for most holdings since analyst data
        # is rare for ETFs and mutual funds)
